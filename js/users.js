      ...
      $.ajax({
        ...
        success: function (data) {
          data.map((item, index) => {
            ...
            const { image, name, id, balance } = item;
            ...
            const renderUsers = `
              <tr class="users__info">
                <td class="users__pos">${index + 1}</td>
                <td class="users__image">
                  ${validImage}
                  <span class="users__name js-name" data-id=${id}>${name}</span>
                </td>
                <td class="users__points">${balance.points}</td>
              </tr>
            `
